...
class LeNet(nn.Module):
    def __init__(self, filter_size, filters_number_1, filters_number_2):
        ...
        filters_num_1 = filters_number_1  # 20
        # filters_num_2 : total number of filters in second 2D convolution layer
        filters_num_2 = filters_number_2
        # in_features (int) – size of each input sample in torch.nn.Linear
        next_layer_size = filters_num_2*(((28//2)//2)**2)
        linear_in_features = next_layer_size
        # out_features (int) – size of each output sample in torch.nn.Linear
        linear_out_features = linear_in_features // 4
        # classes: Total number of unique class labels in our dataset
        classes = 10

        # initialize first set of CONV => RELU => POOL layers
        self.conv1 = nn.Conv2d(numChannels, filters_num_1, kernel_size=filter_size, padding = 'same')
        self.relu1 = nn.ReLU()

        # initialize second set of CONV => RELU => POOL layers
        self.conv2 = nn.Conv2d(filters_num_1, filters_num_2, kernel_size=filter_size , padding='same')
        self.relu2 = nn.ReLU()

        # initialize first (and only) set of FC => RELU layers
        self.fc1 = nn.Linear(
            in_features=linear_in_features, out_features=linear_out_features
        )
        self.relu3 = nn.ReLU()
...
class LeNet_MaxPool_3_Layers(nn.Module):
    def __init__(self, filter_size, filters_number):
        super(LeNet_MaxPool_3_Layers, self).__init__()
        # ─── Parameters ───────────────────────────────────────────────
        # numChannels: The number of channels in the input images (1 for grayscale or 3for RGB)
        numChannels = 1
        # filter_size:  the height and width of the 2D convolution filter matrix
        ###filter_size  = 5
        # filters_num_1 : total number of filters in first 2D convolution layer
        filters_num_1 = filters_number  # 20
        # filters_num_2 : total number of filters in second 2D convolution layer
        filters_num_2 = filters_number
         # filters_num_3 : total number of filters in third 2D convolution layer
        filters_num_3 = filters_number
        # in_features (int) – size of each input sample in torch.nn.Linear
        linear_layer_size = filters_num_3*( ( ( (28//2) //2 )//2 ) **2 )
        linear_in_features = linear_layer_size
        # out_features (int) – size of each output sample in torch.nn.Linear
        ...
        # initialize second set of CONV => RELU => POOL layers
        self.conv3 = nn.Conv2d(filters_num_2, filters_num_3, kernel_size=filter_size, padding='same')
        self.relu3 = nn.ReLU()
