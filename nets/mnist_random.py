...
dataset_lenght = len(train_loader.dataset)-1
...
up = Tile_padding
down  = ( (d+1)*Crop_width - Height )+Tile_padding
left = Tile_padding
right = ( (r+1)*Crop_width - Width )+Tile_padding

#%%
#image_reflected = my_image.reflect(image_np_array, up, down, left , right)
image_reflected = my_image.reflect_mnist(image_np_array, 0, down, 0 , right)
...
# %% Vertical
y= 5
image_translate = np.roll(new_image_original, y, axis=0)
plt.imshow(image_translate)
# %% Horiyontal
y= 5
image_translate = np.roll(new_image_original, y, axis=1)
plt.imshow(image_translate)

# %%
image_transpose = np.transpose(new_image_original)
plt.imshow(image_transpose)

# %%

image_rot90 = np.rot90(new_image_original, k=1, axes=(0, 1))
