...
imgs_masks_file_path = [str(imgs_masks_path / (p.stem + "_OUT.png")) for p in imgs_path.iterdir() if p.is_file()]


imgs_masks_path2= data_path / 'imgs_masks//Without_Padding'
imgs_masks_file_path2 = [str(imgs_masks_path2 / (p.stem + "_OUT.png")) for p in imgs_path.iterdir() if p.is_file()]

#%%
def predict_img(net,
                full_img,
                device,
                scale_factor=img_scale_dv,
                out_threshold=mask_threshold_dv):
    net.eval()
...
def get_args():
    ...
    parser.add_argument('--mask-threshold', '-t', type=float, default = mask_threshold_dv,
                        help='Minimum probability value to consider a mask pixel white')
    parser.add_argument('--scale', '-s', type=float, default = img_scale_dv,
...
if __name__ == '__main__':
    ...
    for k, filename in enumerate(in_files):
        ...
        #Add together
        for i in range (0,v):   
            image_add  = crop(masks[h*i], Tile_Padding,Tile_Width,Tile_Padding,Tile_Width)
            for j in range (1,h):
                img_crop = crop(masks[h*i+j], Tile_Padding,Tile_Width,Tile_Padding,Tile_Width)
        ...
        croped_images2 ,number_crops2 = crop_without_padding(image, (Tile_Width+2*Tile_Padding) )       
        masks2 = {}
